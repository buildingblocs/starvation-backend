...
def runner(left_content: str, right_content: Union[str, None] = None) -> Tuple[bool, Mapping[str, Any]]:
    sandbox_name = f"sandbox-{uuid.uuid4()}"

    with open(fp / "sandbox-deployment.yml") as f:
        dep = yaml.safe_load(f)
        dep["metadata"]["name"] = sandbox_name
        dep["spec"]["containers"][0]["args"] = [left_content, right_content] if right_content else [left_content]
